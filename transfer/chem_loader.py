...
def adjacency_matrix_to_edge_list(adj_matrix):
    ...
    edge_features_list = [[x, 0] for x in edge_features_list_one_dimension]
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        #         data_smiles_list.append(smiles_list[i])
        ...
        elif self.dataset == 'adni705':
            label_path = self.raw_paths[1]     # self.raw_paths=['dataset\\adni705\\raw\\FC', 'dataset\\adni705\\raw\\label_2.csv', 'dataset\\adni705\\raw\\norm_bold_matrix']
            labels = np.loadtxt(label_path, dtype=float)
            print("adni")
            # print(labels)
            print(labels.shape)
            print(type(labels))
            ...
            # bold_path_root = self.raw_paths[0].replace("label_2.csv", "norm_bold_matrix/")
            # bold_matrix_name_list = os.listdir(bold_path_root)
            # for bold_matrix_name in bold_matrix_name_list:
            #     bold_matrix_path = bold_path_root + bold_matrix_name
            #     print(bold_matrix_path)
            #     bold_matrix = np.loadtxt(bold_matrix_path, dtype=float)

            FC_matrix_path_root = self.raw_paths[0] + "/"
            FC_matrix_name_list = os.listdir(FC_matrix_path_root)
            for i in range(len(FC_matrix_name_list)):
                print("graph num", i+1)
                FC_matrix_path = FC_matrix_path_root + "FC_adj_matrix_sample_" + str(i+1) + ".csv"
                FC_matrix = np.loadtxt(FC_matrix_path, dtype=float, delimiter=",")
                # print(FC_matrix)

                edges_list, edge_features_list = adjacency_matrix_to_edge_list(FC_matrix)
                edge_index = torch.tensor(np.array(edges_list).T, dtype=torch.long)
                edge_attr = torch.tensor(np.array(edge_features_list),
                                         dtype=torch.long)

                x = torch.tensor(np.array(FC_matrix), dtype=torch.long)
...
def _load_chembl_with_labels_dataset(root_path):
    ...
    # 1. load folds and labels  3 folds,
...
if __name__ == "__main__":
    ...
    # os.makedirs("dataset/bbbp/processed", exist_ok=True)
    # dataset = MoleculeDataset(root="dataset/bbbp", dataset="bbbp")
    # print(dataset)

    dataset = MoleculeDataset(root="dataset/adni705", dataset="adni705")
    print(dataset)
