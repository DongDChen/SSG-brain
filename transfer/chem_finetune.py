...
from chem_splitters import random_split
from chem_splitters import random_scaffold_split
...
def eval(args, model, device, loader):
    ...
    for i in range(y_true.shape[1]):
        ...
        print("index", i)
        print(y_true[:,i])
        print(np.sum(y_true[:,i] == 1))
        print(np.sum(y_true[:,i] == 0))
...
if __name__ == "__main__":
    ...
    args.input_model_file = os.path.join('../pretrain_models/transfer_adni705_abide', args.cl_model_name)   # transfer_chembl_filtered, transfer_adni705_abide
